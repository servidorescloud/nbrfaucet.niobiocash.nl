...
!function($){
    ...
    var Game = {
        ...
        complete: function(win) {
            $('#status').html("Fim do Jogo, Você " + (win ? "Ganhou - <?php if($linkganhar==""){echo "Mas não foi sorteado! Tente Novamente!"; }else{ echo "$linkganhar";}  ?>" : "Perdeu") + "!");
            if(win) {
                $('#status').addClass("alert-success").removeClass("alert-info").removeClass("alert-danger");
                
       
            }
            else {
                $('#status').removeClass("alert-success").removeClass("alert-info").addClass("alert-danger");
            }
            var el = $("#minesweeper"); 
        ...
        start: function(options){
            ...
            if (numBombs > colls * rows) {
                throw new Error('Muitas bombas');
            }
            
            if(options.skipConfirm || confirm("Tem certeza ?")) {
                Board.reset(rows, colls, numBombs).draw();
                $('#status').html("Jogando!").addClass("alert-info").removeClass("alert-success").removeClass("alert-error");
